	...
	while(c != EOF){
		if(c=='@'){
            ...
            if(ID==test){
                ...
                verif=fscanf(fichier,"%d",&think.coordinates.x);
                if(verif==EOF){
                    printf("error load charging");
                    exit(3);
                    }
                verif=fscanf(fichier,"%d",&think.coordinates.y);
                if(verif==EOF){
                    printf("error load charging");
                    exit(3);
                    }
    ...
    while (in_menu) {
            ...
            switch (menu_cursor) {
                case 0:
                    ...
                    while (!name_complet && !Exit) {
                        ...
                        else if ((48 <= test1 && test1 <= 59) || (97 <= test1 && test1 <= 122) ||
                                 (65 <= test1 && test1 <= 90)) {
                ...
                case 1:
                    ...
                    if (name_complet) {
                        ...
                        if (search_save(name, fichier)) {
                            ...
                            verif = fscanf(fichier, "%d %d %d %d %d %d %d %s %d %d %d %s %d %d ", &player.coordinates.x,
                                           &player.coordinates.y, &player.life, &player.level, &player.XP,
                                           &player.score, &player.strength, player.stuff.name, &player.stuff.id,
                                           &player.stuff.weight, &player.stuff.shield, player.blade.name,
                                           &player.blade.id, &temp);
                            if (verif == EOF) {
                                printf("error load charging");
                                exit(3);
                            }
                            ...
                            switch (temp) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    player.blade.type = MANA;
                                    break;
                            }

                            verif = fscanf(fichier, "%d %d %d %d ", &player.blade.range, &player.blade.dmg,
                            ...
                            //Load room
                            for (int i = 0; i < roomcount; i++) {
                                verif = fscanf(fichier, "%d %d %d %d %d", &tabroom[i].xmin, &tabroom[i].xmax,
                                               &tabroom[i].ymin, &tabroom[i].ymax, &tabroom[i].nbr_enemy);
                                if (verif == EOF) {
                                    printf("error load charging");
                                    exit(3);
                                }
                                ...
                                for (int j = 0; j < tabroom[i].nbr_enemy; j++) {
                                    ...
                                    switch (temp) {
                                        ...
                                        case 4:
                                            ...
                                            break;
                                        case 5:
                                            tabroom[i].tab_enemy[j].name = GOLLUM;
                                            break;
                                        case 6:
                                    ...
                                    verif = fscanf(fichier, "%d %d %d %d %d %d %d ",
                                                   &tabroom[i].tab_enemy[j].coordinates.x,
                                                   &tabroom[i].tab_enemy[j].coordinates.y,
                                                   &tabroom[i].tab_enemy[j].life, &tabroom[i].tab_enemy[j].armor,
                                                   &tabroom[i].tab_enemy[j].strength, &tabroom[i].tab_enemy[j].speed,
                                                   &temp);
                                    if (verif == EOF) {
                                        printf("error load charging");
                                        exit(3);
                                    ...
                                    switch (temp) {
                                        ...
                                        case 2:
                                            ...
                                            break;
                                        case 3:
                                            tabroom[i].tab_enemy[j].type = PRECIOUS;
                                            break;
                                        case 4:
                                            tabroom[i].tab_enemy[j].type = RANGE;
                                            break;
                                    }
                                    verif = fscanf(fichier, "%d", &tabroom[i].tab_enemy[j].weapon.id);
                                    if (verif == EOF) {
                                        printf("error load charging");
                                        exit(3);
                                    }
                                    tabroom[i].tab_enemy[j].weapon = search_blade(tabroom[i].tab_enemy[j].weapon.id,
                                                                                  fichier_blade);
                                }
                                ...
                                for (int j = 0; j < tabroom[i].nbr_object; j++) {
                                    verif = fscanf(fichier, "%d %d ", &tabroom[i].tab_object[j].id,
                                                   &tabroom[i].tab_object[j].stack);
                                    if (verif == EOF) {
                                        printf("error load charging");
                                        exit(3);
                                    }
                                    tabroom[i].tab_object[j] = search_object(tabroom[i].tab_object[j].id,
                                                                             fichier_object);
                                }
                            ...
                            //Load door
                            for (int i = 0; i < doorcount; i++) {
                                verif = fscanf(fichier, "%d %d %d %d %d", &tabdoor[i].x, &tabdoor[i].y,
                                               &tabdoor[i].roomnum1, &tabdoor[i].roomnum2, &tabdoor[i].direction);
                                if (verif == EOF) {
                                    printf("error load charging");
                                    exit(3);
                                }
        ...
        while (in_game) {
            if (!in_pause && !in_inventory) {
                //display game
                display_game(screen_length, screen_width, player, background, timer, time_pause, time_jouer,
                             time_to_play);
                //display donjon
                        ...
                        for (int i = 0; i < doorcount - 1; i++) {
                            if (tabdoor[i].x == player.coordinates.x && tabdoor[i].y - 1 == player.coordinates.y) {
                                ...
                                if (tabdoor[i].roomnum2 == -1) {
                                    //générer room
                                    tabroom[roomcount] = generateroom(seed, &maxroom, tabroom, &tabdoor[i], &roomcount,
                                                                      tabdoor, &doorcount);
//générer mobs armor blade   
                                    tabroom[roomcount - 1].nbr_enemy = 0;
                                    tabroom[roomcount - 1].nbr_object = 0;
                                    tabroom[roomcount - 1].nbr_armor = 0;
                                    tabroom[roomcount - 1].nbr_blade = 0;
                                }
...
//générer mobs armor blade
                    ...
                    collect_armor(&player, tabroom, screen_length, screen_width, background, timer, time_pause,
                                  time_jouer, time_to_play);
                    collect_blade(&player, tabroom, screen_length, screen_width, background, timer, time_pause,
                                  time_jouer, time_to_play);
                ...
                if (test1 == key_exit) {
                    ...
                    verif2 = fprintf(fichier, "%d %d %d %d %d %d %d %s %d %d %d %s %d ", player.coordinates.x,
                                     player.coordinates.y, player.life, player.level, player.XP, player.score,
                                     player.strength, player.stuff.name, player.stuff.id, player.stuff.weight,
                                     player.stuff.shield, player.blade.name, player.blade.id);
                    if (verif2 == EOF) {
                        printf("error load charging");
                        exit(5);
                    }
                    ...
                    verif2 = fprintf(fichier, "%d %d %d %d ", player.blade.range, player.blade.dmg, player.blade.weight,
                                     player.nbr_object_inventory);
                    if (verif2 == EOF) {
                        printf("error load charging");
                        exit(3);
                    ...
                    //save room
                    for (int i = 0; i < roomcount; i++) {
                        verif2 = fprintf(fichier, "%d %d %d %d %d ", tabroom[i].xmin, tabroom[i].xmax, tabroom[i].ymin,
                                         tabroom[i].ymax, tabroom[i].nbr_enemy);
                        if (verif2 == EOF) {
                            printf("error load charging");
                            exit(3);
                        }
                        ...
                        for (int j = 0; j < tabroom[i].nbr_enemy; j++) {
                            ...
                            verif2 = fprintf(fichier, "%d %d %d %d %d %d ", tabroom[i].tab_enemy[j].coordinates.x,
                                             tabroom[i].tab_enemy[j].coordinates.y, tabroom[i].tab_enemy[j].life,
                                             tabroom[i].tab_enemy[j].armor, tabroom[i].tab_enemy[j].strength,
                                             tabroom[i].tab_enemy[j].speed);
                            if (verif2 == EOF) {
                                printf("error load charging");
                                exit(3);
                            }
                            ...
                            verif2 = fprintf(fichier, "%d", tabroom[i].tab_enemy[j].weapon.id);
                            if (verif2 == EOF) {
                                printf("error load charging");
                                exit(3);
                            }
                        ...
                        //save object
                        for (int j = 0; j < tabroom[i].nbr_object; j++) {
                            verif2 = fprintf(fichier, "%d %d ", tabroom[i].tab_object[j].id,
                                             tabroom[i].tab_object[j].stack);
                            if (verif2 == EOF) {
                                printf("error load charging");
                                exit(3);
                    ...
                    //save door
                    for (int i = 0; i < doorcount; i++) {
                        verif2 = fprintf(fichier, "%d %d %d %d %d ", tabdoor[i].x, tabdoor[i].y, tabdoor[i].roomnum1,
                                         tabdoor[i].roomnum2, tabdoor[i].direction);
                        if (verif2 == EOF) {
                            printf("error load charging");
                            exit(3);
                        }
